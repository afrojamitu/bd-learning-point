...
const Withdrawal = () => {
  ...
  if (!isLoading && withdrawals?.data?.data?.withdrawals?.length > 0)
    content = withdrawals?.data?.data?.withdrawals?.map((withdraw, key) => (
      <tr
        key={key}
        className="bg-white border-b hover:bg-gray-50 borderBg"
      >
        <td
          data-label="SL"
          className="block md:table-cell text-end md:text-start md:before:content-[''] before:content-[attr(data-label)] before:float-left before:font-bold font-medium text-gray-900 whitespace-nowrap px-6 p-4"
        >
          {key++}
        </td>
        <th
          ...
          className="block md:table-cell text-end md:text-start md:before:content-[''] before:content-[attr(data-label)] before:float-left before:font-bold font-medium text-gray-900 whitespace-nowrap px-6 py-4 font-medium text-gray-900 whitespace-nowrap"
        >
          {withdraw?.transection_id}
        </th>
        <td
          data-label="Amount"
          className="block md:table-cell text-end md:text-start md:before:content-[''] before:content-[attr(data-label)] before:float-left before:font-bold font-medium text-gray-900 whitespace-nowrap px-6 py-4"
        >
          {withdraw?.amount}
        </td>
        <td
  ...
  return (
    <>
      ...
      <div className="container mx-auto my-5 p-5">
        <div className="p-5 mb-2 shadow-md rounded-lg bg-gradient-to-l from-pink-300 to-yellow-400">
            ...
            <div className="flex justify-between items-center">
              ...
              {!isFetching && (
                <button
                  ...
                  className="block text-white bg-gradient-to-r from-green-400 to-blue-500 hover:from-pink-500 hover:to-yellow-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                  type="button"
                >
                  Withdraw Now
                </button>
